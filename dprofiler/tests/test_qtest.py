...
def test_min_number_of_records(checks:QTest,
                                test_profile:Dict) -> None:
    assert checks.check_min_number_of_records(test_profile=test_profile,
                                               min_threshold=100) == True 
    
def test_max_number_of_records(checks:QTest,
                               test_profile:Dict) -> None:
    assert checks.check_max_number_of_records(test_profile=test_profile,
                                              max_threshold=1000) == False
    
def test_if_numeric_below_thresh(checks:QTest,
                                 test_profile:Dict) -> None:
    assert checks.check_numeric_below_thresh(test_profile=test_profile,
                                             min_thresh=15,
                                             col="termdays") == True
